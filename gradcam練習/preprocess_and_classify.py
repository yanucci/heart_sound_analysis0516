...
def create_stft_spectrogram(audio, sr=5000, n_fft=256, hop_length=64):
    """
    STFTスペクトログラム（絶対値）を生成し、0-1正規化して返す
    """
    stft = librosa.stft(audio, n_fft=n_fft, hop_length=hop_length)
    spec = np.abs(stft)
    # 周波数0-5000Hzのみ抽出（sr/2が最大）
...
def grad_cam(model, img_array, layer_name):
    """
    Grad-CAMのヒートマップを返す
    img_array: (H, W, 1) の入力画像
    layer_name: 可視化したい畳み込み層の名前
    """
...
def visualize_gradcam_on_stft(model, wav_path, layer_name, sr=5000, n_fft=256, hop_length=64, save_path=None):
    ...
    # Grad-CAM用の入力（0-1正規化, チャンネル次元追加, モデル入力shapeにリサイズ）
    spec = np.abs(D)
    spec_norm = (spec - spec.min()) / (spec.max() - spec.min() + 1e-8)
...
def visualize_gradcam_on_stft_single(model, wav_path, layer_name, sr=5000, n_fft=256, hop_length=64, save_path=None):
    ...
    plt.imshow(heatmap_mask, cmap='jet', alpha=0.8, aspect='auto',
               extent=[times[0], times[-1], freqs[0], freqs[-1]], origin='lower', vmin=0, vmax=1)
...
def visualize_gradcam_on_stft_masked(model, wav_path, layer_name, sr=5000, n_fft=256, hop_length=64, save_path=None, threshold=0.5):
    ...
    plt.colorbar(format='%+2.0f dB', label='dB')
    plt.imshow(np.ma.masked_where(~mask, heatmap_resized), cmap='jet', alpha=0.8, aspect='auto',
               extent=[times[0], times[-1], freqs[0], freqs[-1]], origin='lower', vmin=0, vmax=1)
    cbar = plt.colorbar(mpl.cm.ScalarMappable(cmap='jet'), ax=plt.gca(), orientation='vertical', pad=0.02)
    cbar.set_label('Grad-CAM')
    plt.tight_layout()
...
def save_gradcam_heatmap_only(model, wav_path, layer_name, sr=5000, n_fft=256, hop_length=64, save_path=None):
    audio = load_and_preprocess_audio(wav_path, target_sr=sr)
    D = librosa.stft(audio, n_fft=n_fft, hop_length=hop_length)
    S_db = librosa.amplitude_to_db(np.abs(D), ref=np.max)
    freqs = librosa.fft_frequencies(sr=sr, n_fft=n_fft)
...
# フォルダ内全ファイル一括処理（ヒートマップ単体）
def batch_gradcam_heatmap_only_on_folder(model, folder_path, layer_name, out_folder):
    if not os.path.exists(out_folder):
        os.makedirs(out_folder)
    wav_files = [f for f in os.listdir(folder_path) if f.endswith('.wav')]
    for wav_file in wav_files:
        ...
        save_gradcam_heatmap_only(model, wav_path, layer_name, save_path=save_path)
...
if __name__ == "__main__":
    ...
    # Grad-CAM可視化（マスク方式で全ファイル保存）
    out_folder = '/Users/yusuke.yano/python_practice/heart_sound_analysis/data/segments/gradcam_masked'
    batch_gradcam_masked_on_folder(model, folder_path, layer_name, out_folder, threshold=0.5)
    # Grad-CAMヒートマップ単体（全ファイル保存）
    out_folder_heatmap = '/Users/yusuke.yano/python_practice/heart_sound_analysis/data/segments/gradcam_heatmap_only'
